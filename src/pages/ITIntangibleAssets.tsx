...
import { Plus, Trash2, Save, RotateCcw, Loader2, Search, Upload, Download, FileDown, RotateCw } from 'lucide-react';
import { parseCsvFile, mapIntangibleCsvRows, downloadIntangibleCsv, downloadIntangibleTemplate } from '@/utils/csv';
...
export default function ITIntangibleAssets() {
  ...
  const { rows, addRow, addRows, updateCell, markDeleted, reset, forceSync, dirtyStats, hasDirty, getChanges } = useGridEditor<IntangibleAssetRow>(
    assets,
    {
      idField: 'id',
      newRowTemplate: () => {
  ...
  const getDisplayValue = useCallback((row: GridRow<IntangibleAssetRow>, col: ColDef): string => {
  ...
  const visibleRows = useMemo(() => {
    ...
    if (search) {
      ...
      filtered = filtered.filter(r => columns.some(col => String((r.data as any)[col.key] || '').toLowerCase().includes(s)));
    }
    if (sortKey && sortDir) {
  ...
  const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = mapIntangibleCsvRows(csvRows, { departments, assetTypes: assetTypes || [] });
  ...
  const handleCsvDownload = () => {
    ...
    downloadIntangibleCsv(dataRows, { departments, assetTypes: assetTypes || [] });
  };
  ...
  const renderCell = (row: GridRow<IntangibleAssetRow>, col: ColDef) => {
    ...
    return (
      <input type={col.type === 'number' ? 'number' : 'text'} value={val ?? ''} disabled={disabled}
        ...
        className={`w-full min-w-[60px] bg-transparent px-1 py-0.5 text-xs text-foreground disabled:opacity-40 focus:outline-none focus:ring-1 focus:ring-primary rounded ${col.required && !val ? 'ring-1 ring-destructive' : ''}`} />
    );
  };
  ...
  const rowBg = (row: GridRow<IntangibleAssetRow>) => {
    ...
    <div className="space-y-4">
      ...
      {canEdit && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={downloadIntangibleTemplate} className="flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
