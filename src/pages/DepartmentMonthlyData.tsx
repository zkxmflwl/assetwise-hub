...
export default function DepartmentMonthlyData() {
  ...
  return (
    <div className="space-y-4">
      ...
      {canEdit && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-medium text-white hover:opacity-90 disabled:opacity-40 transition-opacity"
          >
          ...
          {hasDirty && (
            <div className="flex gap-2 text-xs">
              {dirtyStats.added > 0 && (
                <span className="rounded bg-emerald-100 px-2 py-1 text-emerald-700">추가 {dirtyStats.added}</span>
              )}
              {dirtyStats.updated > 0 && (
                <span className="rounded bg-amber-100 px-2 py-1 text-amber-700">수정 {dirtyStats.updated}</span>
              )}
              {dirtyStats.deleted > 0 && (
                <span className="rounded bg-red-100 px-2 py-1 text-red-700">삭제 {dirtyStats.deleted}</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="text-xs text-muted-foreground">{deptName} 이연 매출 / 매입은 1월 데이터에만 기입하며, 전년도에서 이연된 매출/매입을 뜻합니다.{salesData.length}건</div>
      <div className="text-xs text-muted-foreground">{deptName} *2026년까지는 기입하지 않습니다. {salesData.length}건</div>
            ...
            <tbody>
              {visibleRows.length === 0 ? (
                ...
                visibleRows.map((row) => (
                  ...
                  >
                    {canEdit && (
                      <td className="w-8 border-r border-border/50 px-2 py-1.5 align-top">
                        <input
                          type="checkbox"
                    ...
                    {columns.map((col) => (
                      <td key={col.key} className="border-r border-border/50 last:border-r-0 px-3 py-1.5 align-top">
                        {renderCell(row, col)}
                      </td>
                    ))}
                  </tr>
